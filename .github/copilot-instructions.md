...
- **Mobile playback** — touch devices skip autoplay, offscreen preloading, iframe transforms, overlays, and the YouTube JavaScript player API. Pointer events remain enabled so one tap starts playback reliably in WebKit, while the control bar stays hidden.
