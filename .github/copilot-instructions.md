...
- **Mobile playback** — primary touch/no-hover devices preload a visually hidden YouTube iframe behind a SignSpark facade. The button enables after iframe load and its user gesture sends `playVideo`; controls and iframe pointer events remain disabled. Touch-capable desktops keep the direct, autoplaying desktop player.
